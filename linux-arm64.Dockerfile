...
ENV IMAGE_STATS=${IMAGE_STATS} BUILD_ARCHITECTURE=${BUILD_ARCHITECTURE} \
    ...
    VPN_ENABLED="false" VPN_CONF="wg0" VPN_PROVIDER="generic" VPN_LAN_NETWORK="" VPN_LAN_LEAK_ENABLED="false" VPN_EXPOSE_PORTS_ON_LAN="" VPN_AUTO_PORT_FORWARD="true" VPN_AUTO_PORT_FORWARD_TO_PORTS="" VPN_FIREWALL_TYPE="auto" VPN_HEALTHCHECK_ENABLED="false" PRIVOXY_ENABLED="false" UNBOUND_ENABLED="false" \
...
# install packages
RUN apk add --no-cache bash ca-certificates coreutils curl dos2unix findutils grep ipcalc iproute2 iptables iptables-legacy jq libcap-utils nftables outils-rs p7zip privoxy python3 sed shadow tzdata unbound unzip wget wireguard-tools && \
